...
const workElement = document.querySelectorAll("#my_work .work-items .wrap");

const workModal = new bootstrap.Modal("#workModal");
// workModal.show(); // Must be removed
workElement.forEach((item) => {
  item.addEventListener("click", function () {
...
let testemonialImages = document.querySelectorAll("#testemonial .images img");
testemonialImages.forEach((item, index) => {
  let position = index + 1;
  item.addEventListener("click", function () {
    document
      .querySelector("#testemonial .images img.active")
      .classList.remove("active");
    document
      .querySelector(`#testemonial .images img:nth-child( ${position})`)
      .classList.add("active");

    document
      .querySelector("#testemonial .comments .item.active")
      .classList.remove("active");
    document
      .querySelector(`#testemonial .comments .item:nth-child( ${position})`)
      .classList.add("active");
  });
});

let contactFormItem = document.querySelectorAll(
  "#contact_me form input, #contact_me form textarea"
);
contactFormItem.forEach((item) => {
